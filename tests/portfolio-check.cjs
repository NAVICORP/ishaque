...
(async () => {
  ...
  for (const viewport of [
    ...
    if (isMobile) await page.locator('.quote-section').screenshot({ path: `_artifacts/quote-${viewport.name}.jpg`, type: 'jpeg', quality: 90 });
    await page.locator('.contact-section').scrollIntoViewIfNeeded();
    await page.waitForTimeout(isMobile ? 80 : 700);
    ...
    const contactFormAnchorValid = await contactForm.getAttribute('id') === 'project-form'
      && await contactForm.evaluate(form => Number.parseFloat(getComputedStyle(form).scrollMarginTop) >= 90);
    ...
    const startProjectTargetsForm = startProjectTargets.length === 2 && startProjectTargets.every(href => href === '#project-form');
    ...
    const contactDesktopSideBySide = isMobile || await page.evaluate(() => {
      ...
      return form.left >= copy.right + 40 && Math.abs(form.top - copy.top) < 80;
    });
    const contactMobileStacked = !isMobile || await page.evaluate(() => {
      const copy = document.querySelector('.contact-copy').getBoundingClientRect();
      const form = document.querySelector('.contact-form').getBoundingClientRect();
      return form.top >= copy.bottom + 32 && Math.abs(form.left - copy.left) <= 2 && Math.abs(form.right - copy.right) <= 2;
    });
    const contactSectionHeight = await page.locator('.contact-section').evaluate(element => Math.round(element.getBoundingClientRect().height));
  ...
  for (const viewport of [
    ...
    const portfolioImagesWidescreen = await page.locator('.portfolio-image').evaluateAll(elements => elements.length === 55 && elements.every(element => getComputedStyle(element).aspectRatio === '16 / 9') && elements.filter(element => !element.closest('.portfolio-card').hidden).every(element => {
      const rect = element.getBoundingClientRect();
      return Math.abs(rect.width / rect.height - 16 / 9) < .02;
    }));
    const whatsappHref = await page.locator('.whatsapp-float').getAttribute('href');
    const portfolioIconsValid = await page.locator('.portfolio-meta i').evaluateAll(elements => elements.length === 55 && elements.every(element => {
      const style = getComputedStyle(element);
