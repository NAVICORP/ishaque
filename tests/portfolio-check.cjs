...
(async () => {
  ...
  for (const viewport of [
    ...
    const portfolioImagesWidescreen = await page.locator('.portfolio-image').evaluateAll(elements => elements.length === 53 && elements.every(element => getComputedStyle(element).aspectRatio === '16 / 9') && elements.filter(element => !element.closest('.portfolio-card').hidden).every(element => {
      const rect = element.getBoundingClientRect();
      return Math.abs(rect.width / rect.height - 16 / 9) < .02;
    }));
    const whatsappHref = await page.locator('.whatsapp-float').getAttribute('href');
    const portfolioIconsValid = await page.locator('.portfolio-meta i').evaluateAll(elements => elements.length === 53 && elements.every(element => {
      const style = getComputedStyle(element);
  ...
  const failed = results.some(result => {
    ...
    const expectedOgImage = result.page === 'projects'
      ? { url: 'https://ishaquenv.com/assets/portfolio/projects-og-image.png?v=20260827', width: '1200', height: '630' }
